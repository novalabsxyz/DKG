...
Application::Application(SystemType   systemtype,
    ...
    , sysparams(pairingparamfile, sysparamfile)
    , buddyset(sysparams, certfile, keyfile)
    , ph(ph)
    , timeout_times(0)
{
    // Ignore SIGPIPE
    ...
    if (systemtype != BLS_CLIENT)
    {
        ...
        int one = 1;
        int res = setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one));
        if (res < 0)
        {
            perror("setsockopt");
...
Message *
Application::get_next_message(BuddyID & buddyID, BuddyID selfID)
{
    while (1)
    {
...
void
Application::measure_init()
{
    gettimeofday(&start_time, NULL);
}

void
Application::measure_now()
{
    struct timeval now;
    gettimeofday(&now, NULL);

    long realms = (now.tv_sec - start_time.tv_sec) * 1000 + (now.tv_usec - start_time.tv_usec) / 1000;
    ...
    IStream << NodeID(buddyset.get_my_id());
...
unsigned long
Application::get_time_diff(int t, unsigned long incre)
{
    ...
    if (timeout_times == 1)
    {
        ...
        last_timeout  = (now.tv_sec - start_time.tv_sec) * 1000 + (now.tv_usec - start_time.tv_usec) / 1000;
