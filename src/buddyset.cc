...
BuddySet::BuddySet(const SystemParam & sysparams, const char * certfilename, const char * keyfilename)
    : sysparams(sysparams)
{
    my_dsa_privkey = NULL;
    if (keyfilename)
    {
        ...
        gnutls_x509_privkey_export_dsa_raw(my_privkey, &pd, &qd, &gd, &yd, &xd);
        ...
        gcry_sexp_build(&my_dsa_privkey, NULL, "(private-key(dsa(p%m)(q%m)(g%m)(y%m)(x%m)))", p, q, g, y, x);
        gcry_mpi_release(p);
        gcry_mpi_release(q);
        gcry_mpi_release(g);
        gcry_mpi_release(y);
...
void
BuddySet::init_contact_list(const char * filename)
{
    if (filename == NULL)
        return;
    ...
    while (1)
    {
        ...
        int  res =
            sscanf(nextline.data(), "%d %d.%d.%d.%d %d %s %c", &id, &a1, &a2, &a3, &a4, &port, certfilename, &leader_char);
        if ((res == 8) && (leader_char == 'L'))
        {
            isleader = true;
            leader   = id;
        }
        ...
        if (certfile.good())
        {
            ...
            cerr << "Certificate doesn't exist for " << id << "\n";
    }
    // If no Leader character (L) present, select first node in the contact list as the one
    if (!isleader)
        leader = (contactlist.begin())->first;
    file.close();
}

void
BuddySet::set_leader(BuddyID new_leader)
{
...
BuddySet::set_fds(fd_set * fdsp)
{
    ...
    for (map<int, Buddy *>::iterator fditer = fdmap.begin(); fditer != fdmap.end(); fditer++)
    {
        //    	cerr << "set dsp: " << fditer->first << endl;
        FD_SET(fditer->first, fdsp);
        if (fditer->first > maxfd)
            maxfd = fditer->first;
    }
    return maxfd;
}
...
Buddy *
BuddySet::find_set_fd(fd_set * fdsp)
{
    ...
    if (FD_ISSET(notifyids[0], fdsp))
    {
        ...
        memmove(&newBuddy, buf, sizeof(newBuddy));
        if (find_buddy(newBuddy->get_id()) == NULL)
        {
...
void
BuddySet::add_buddy_id(Buddy * buddy)
{
...
Buddy *
BuddySet::find_buddy_id(BuddyID id) const
{
    map<BuddyID, Buddy *>::const_iterator found;
    found = idmap.find(id);
    if (found == idmap.end())
...
void
BuddySet::notify_add_buddy_id(Buddy * buddy)
{
...
void
BuddySet::notify_add_buddy_fd(int fd, BuddyID id)
{
...
Buddy *
BuddySet::add_buddy_fd(int fd, BuddyID id)
{
...
void
BuddySet::close_buddy(Buddy * buddy)
{
    int fd = buddy->get_fd();
    fdmap.erase(fd);
    buddy->close_fd();
...
void
BuddySet::del_buddy(Buddy * buddy)
{
    int     fd = buddy->get_fd();
...
Buddy *
BuddySet::find_buddy(BuddyID id, int contact)
{
...
void
BuddySet::send_message(BuddyID id, const NetworkMessage & message)
{
...
void
BuddySet::sign(const unsigned char * data, size_t len, unsigned char * sig) const
{
    // First hash the data
    ...
    memset(sig, 0, 40);
    gcry_mpi_print(GCRYMPI_FMT_USG, (unsigned char *)sig + (20 - nr), nr, NULL, r);
    gcry_mpi_print(GCRYMPI_FMT_USG, (unsigned char *)sig + 20 + (20 - ns), ns, NULL, s);
    gcry_mpi_release(r);
    gcry_mpi_release(s);
}
