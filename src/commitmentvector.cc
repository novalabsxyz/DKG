...
// Copy constructor
CommitmentVector::CommitmentVector(const CommitmentVector & vec)
    : indices(vec.getIndices())
{
    ...
    for (it = shV.begin(); it != shV.end(); ++it)
    {
        G1 entry(*it);
        shares.push_back(entry);
    }
    for (it = subshV.begin(); it != subshV.end(); ++it)
    {
        G1 entry(*it);
        subshares.push_back(entry);
    }
    for (itstr = strV.begin(); itstr != strV.end(); ++itstr)
    {
        string str(*itstr);
...
// Deserialize
CommitmentVector::CommitmentVector(const SystemParam & sys, const unsigned char *& buf, size_t & len)
{
    G1 U = sys.get_U();
    indices.clear();
    shares.clear();
    ...
    // read indices
    NodeIDSize indexcnt;
    read_us(buf, len, indexcnt);
    for (NodeIDSize j = 0; j < indexcnt; ++j)
    {
        NodeID index;
        read_us(buf, len, index);
        indices.push_back(index);
    }

    // read shares
    NodeIDSize sharecnt;
    read_us(buf, len, sharecnt);
    ...
    // read hashes
    NodeIDSize hashcnt;
    read_us(buf, len, hashcnt);
    for (NodeIDSize j = 0; j < hashcnt; ++j)
    {
        string hash;
        read_str(buf, len, hash, HashSize);
        hashes.push_back(hash);
    }

    // read subshares
    NodeIDSize subsharecnt;
    read_us(buf, len, subsharecnt);
    if (!(subsharecnt))
        return;
...
string
CommitmentVector::toString(bool includeSubshares) const
{
    string                         returnStr;
    ...
    if (includeSubshares)
    {
        ...
        write_us(returnStr, (unsigned short)0);
    return returnStr;
}

bool
CommitmentVector::operator==(const CommitmentVector & rhs) const
{
    ...
    if (shares != rhs.shares)
        return false;
    if (hashes != rhs.hashes)
        return false;
    return true;
}

void
CommitmentVector::setSubshares(const vector<G1> & subshares)
{
...
void
CommitmentVector::setSubshares(const SystemParam & sys, const vector<Zr> & values)
{
    G1 U = sys.get_U();
    subshares.clear();
...
bool
CommitmentVector::checkPoly(NodeID verifierID) const
{
    if (!(subshares[0] == shares[verifierID]))
    {
        cerr << "Error with share comparison\n";
        return false;
    }
...
bool
CommitmentVector::verifyPoly(const SystemParam & sys, NodeID verifierID, const Polynomial & poly)
{
    vector<NodeID>::const_iterator it2d;
    G1                             U = sys.get_U();
    subshares.clear();
...
bool
CommitmentVector::verifyPoint(const SystemParam & sys, NodeID senderID, NodeID verifierID, const Zr & point) const
{
    G1 U = sys.get_U();
    ...
    if (!checkPoly(senderID))
    {
        cerr << "Error with CheckPoly\n";
        return false;
    }
    return true;
}
...
void
CommitmentVector::dump(FILE * f, unsigned int indent) const
{
