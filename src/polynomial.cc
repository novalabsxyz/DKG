...
Polynomial::Polynomial(const SystemParam & sys, unsigned int t, const Zr & index, const Zr & term)
{
    for (unsigned int i = 0; i <= t; ++i)
    {
        Zr randcoeff(sys.get_Pairing(), true);
...
static void
copy_elt(Zr & a, const Zr b)
{
    a = b;
}
static void
add_elt(Zr & a, const Zr b)
{
    a += b;
}
static void
sub_elt(Zr & a, const Zr b)
{
    a -= b;
}

Polynomial &
Polynomial::merge(const Polynomial & rhs, void (*mfunc)(Zr &, const Zr))
{
...
// element_t coeff
const Zr
Polynomial::getCoeff(unsigned int i) const
{
    if (i < coeffs.size())
        return coeffs[i];
    else
...
const Zr
Polynomial::operator()(const Zr & x) const
{
...
void
Polynomial::dump(FILE * f, char * label, unsigned short base) const
{
    if (label)
        fprintf(f, "%s: ", label);
    fprintf(f, "[ ");
