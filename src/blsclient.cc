...
{
  public:
    BLSClient(const char * pairingfile,
    ...
    while (1)
    {
        ...
        case NETWORK:
        {
            ...
            case BLS_SIGNATURE_RESPONSE:
            {
                ...
                if (buddyID == 2 || buddyID == 3 || buddyID == 5 || buddyID == 8 || buddyID == 17 || buddyID == 24
                    || buddyID == 35
                    || buddyID == 32
                    || buddyID == 33
                    || buddyID == 37
                ...
                if (ph > signResponse->ph)
                {
                    ...
                    cerr << "Phase for  BLS signature response message" << signResponse->ph
                ...
                else if (ph < signResponse->ph)
                {
                    // TODO: Actual system need not to display the following messages.
                    cerr << "Phase for  BLS signature response message" << signResponse->ph
                         << " is newerolder than the current phase " << ph << endl;
                }
                else
                {
                    ...
                    if (((NodeID)signatureShares.size() == 2 * sysparams.get_t() + 1) && !signature.isElementPresent())
                    {
                        ...
                        for (map<NodeID, G1>::const_iterator it = signatureShares.begin(); it != signatureShares.end();
                             ++it)
                        {
            ...
            case VERIFIED_BLS_SIGNATURES:
            {
                ...
                for (map<NodeID, G1>::const_iterator it = verifiedSign->signatures.begin();
                     it != verifiedSign->signatures.end();
                     ++it)
                {
                    indices.push_back(Zr(e, (signed long)it->first));
                    shares.push_back(it->second);
...
main(int argc, char ** argv)
{
    ...
    if (argc != 5)
    {
        cerr << "Usage: " << argv[0] << "certfile keyfile contactlist phase\n";
        exit(1);
    }
