...
string system_param_part3 = "U "
                            "[15442193728886989279518909381092123512247646383937532607359197105636593043460247018674238"
                            "5706265523539972032311143735573288479871818380689794065934739646,"
                            "105322435950956402192311181969163570058367651725093433050155157836463104684465240582076780"
                            "1196558009238591149557814028743651030702748906271871797495725636]";
...
main()
{
    ...
    for (int i = 0; i < testcases.size(); ++i)
    {
        ...
        if (testcases[i].n != prev_n || testcases[i].t != prev_t || testcases[i].f != prev_f)
        {
        ...
        sprintf(buf,
                ...
                testcases[i].t,
                testcases[i].f,
                testcases[i].non_responsive_server_num);
        string paradir(buf);
        ...
        for (int j = 0; j < testcases[i].times; ++j)
        {
            ...
            {
                sprintf(buf,
                        "ssh -n uwaterloo_dkg2@%s ./node%s 9900 certs/%d.pem certs/%d-key.pem contlist 0 %d %d&",
                        used_nodes[k].data(),
                        version_chars,
                        k + 1,
                        k + 1,
            ...
            while (1)
            {
                ...
                {
                    sprintf(buf,
                            "scp -r uwaterloo_dkg2@%s:./message.log logdir/message_%d.log > /dev/null&",
                            used_nodes[k].data(),
                            k + 1);
                    system(buf);
                    sprintf(buf,
                            "scp -r uwaterloo_dkg2@%s:./dkg*.log logdir/dkg_%d.log > /dev/null&",
                            used_nodes[k].data(),
                            k + 1);
                    system(buf);
                    sprintf(buf,
                            "scp -r uwaterloo_dkg2@%s:./timeout.log logdir/timeout_%d.log > /dev/null&",
                            used_nodes[k].data(),
                            k + 1);
                    system(buf);
                }
