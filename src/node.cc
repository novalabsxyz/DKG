...
class Node : public Application
{
  public:
    Node(const char *   pairingfile,
         ...
         Phase          ph,
         CommitmentType commType = Feldman_Matrix,
         int            nrln     = 0)
        ...
        , timeoutLog("timeout.log", ios::out)
    {
        //  		 Application(NODE, pairingfile, sysparamfile, listen_addr, listen_port, certfile, keyfile,
        //  contactlistfile, ph){
        ...
        nodeState = (ph == 0) ? FUNCTIONAL : UNDER_RECOVERY;
        if (selfID == buddyset.get_leader())
            nodeState = LEADER_UNCONFIRMED;
  ...
  private:
    ...
    map<LeaderChangeMessage, map<NodeID, string>, LeaderChangeMessageCmp>
           leaderChangeMsg; // LeaderChange message received
...
Node::run()
{
    ...
    cout << "-Run node " << selfID << " starting at " << now.tv_sec << "." << setw(6) << now.tv_usec
    ...
    while (timeoutValueStream >> line_indicator)
    {
        ...
        if (t_n == sysparams.get_n() && t_t == sysparams.get_t() && t_f == sysparams.get_f())
        {
    ...
    if (!ph)
    { // ph =0; Send LeaderChange to the leader, which it may use with the DKGSend
        ...
        gettimeofday(&now, NULL);
        msgLog << "LEADER_CHANGE " << leadChg.get_ID() << " for "
               << "* SENT from " << selfID << " to " << buddyset.get_leader() << " at " << now.tv_sec << "." << setw(6)
        ...
        if (selfID == buddyset.get_leader())
        {
            msgLog << "* I am ready to receive at " << now.tv_sec << "." << setw(6) << now.tv_usec << endl;
        }
        ...
        for (iter = activeNodes.begin(); iter != activeNodes.end(); ++iter)
        {
            ...
            gettimeofday(&now, NULL);
            msgLog << "VSS_HELP " << vssHelp.get_ID() << " for "
                   << "* SENT from " << selfID << " to " << *iter << " at " << now.tv_sec << "." << setw(6)
                   << now.tv_usec << endl;
        }
    }
    ...
    while (1)
    {
        ...
        case USER:
        {
            ...
            case STATE_INFORMATION:
            {
                ...
                {
                case ID:
                    cerr << "ID for node is " << selfID;
                    break;
                case N:
                    cerr << "n is " << sysparams.get_n();
                    break;
                case T:
                    cerr << "t is " << sysparams.get_t();
                    break;
                case F:
                    cerr << "f is " << sysparams.get_f();
                    break;
                case U:
                    (sysparams.get_U()).dump(stderr, (char *)"U is ", 10);
                    break;
                case STATE:
                    cerr << "Node is ";
                    switch (nodeState)
                    {
                    case LEADER_UNCONFIRMED:
                        cerr << "not yet confirmed.";
                        break;
                    case UNDER_RECOVERY:
                        cerr << "under recovery.";
                        break;
                    ...
                    case AGREEMENT_STARTED:
                        cerr << "in state Agreement_Started.";
                        break;
                    case LEADER_CHANGE_STARTED:
                        cerr << "in state LeaderChange_Started.";
                        break;
                    case AGREEMENT_COMPLETED:
                        cerr << "in state Agreement_Completed.";
                        break;
                    case DKG_COMPLETED:
                        cerr << "in state DKG_Completed.";
                        break;
                    }
                    break;
                case PHASE:
                    ...
                    break;
                case LEADER:
                    cerr << "Current leader is " << buddyset.get_leader();
                    break;
                case COMMITMENT:
                    cerr << "Commitment is" << endl;
                    result.C.dump(stderr);
                    break;
                case MYSHARE:
                ...
                case NONE:
                default:
                    cerr << "State Information Request: Not Well-Formatted";
                    break;
                }
                cerr << endl;
            }
        ...
        case NETWORK:
        {
            ...
            case NET_MSG_PING:
            {
                ...
                bool                  valid   = read_sig(buddy,
                                      bodyptr,
                                      bodylen,
                                      (const unsigned char *)pnm->strMsg.data(),
                                      (const unsigned char *)(pnm->strMsg).data() + (pnm->strMsg).length());
                if (valid)
                {
            ...
            case VSS_SEND:
            {
                ...
                gettimeofday(&now, NULL);
                msgLog << "VSS_SEND " << vssSend->get_ID() << " for "
                       << "* RECEIVED from " << buddyID << " to " << selfID << " at " << now.tv_sec << "." << setw(6)
                       << now.tv_usec << endl;
                // if (selfID == buddyset.get_leader()) {
                //}
                if (ph > vssSend->ph)
                ...
                else if (ph < vssSend->ph)
                {
                    ...
                    // Do it when I make decision about the recovery set B
                         ...
                         // condition below make sure that if the DKG is complete, then VSS only for NodeID the decided
                         // set continue
                         ((nodeState != AGREEMENT_COMPLETED)
                          || (find(DecidedVSSs.begin(), DecidedVSSs.end(), buddyID) != DecidedVSSs.end())))
                {
                    ...
                    if (vssSend->C.verifyPoly(sysparams, selfID, vssSend->a))
                    {
                        ...
                        if (SendReceived.find(buddyID) == SendReceived.end())
                        {
                            ...
                            for (iter = activeNodes.begin(); iter != activeNodes.end(); ++iter)
                            {
                                ...
                                gettimeofday(&now, NULL);
                                msgLog << "VSS_ECHO " << vssEcho.get_ID() << " for " << vssEcho.dealer << " SENT from "
                                       << selfID << " to " << *iter << " at " << now.tv_sec << "." << setw(6)
                                       << now.tv_usec << " standard 1" << endl;
                            }
                        }
                        else
                        {
                          ...
                          // for(it = C.begin(); it != C.end(); ++it){ cerr<<"BuddyID is"<<it->first<<endl;
                        // it->second.dump(stderr);}
                    }
                    else
                        cerr << "Error with the VSSsend message received at " << selfID << " from " << buddyID << endl;
                }
            }
            break;
            case VSS_ECHO:
            {
                ...
                gettimeofday(&now, NULL);
                msgLog << "VSS_ECHO " << vssEcho->get_ID() << " for " << vssEcho->dealer << " RECEIVED from " << buddyID
                       << " to " << selfID << " at " << now.tv_sec << "." << setw(6) << now.tv_usec << endl;
                ...
                else if (ph < vssEcho->ph)
                {
                    ...
                    // Do it when I make decision about the recovery set B
                         ...
                         // set continue
                         ((nodeState != AGREEMENT_COMPLETED)
                          || (find(DecidedVSSs.begin(), DecidedVSSs.end(), vssEcho->dealer) != DecidedVSSs.end())))
                {
                    if (vssEcho->C.verifyPoint(sysparams, buddyID, selfID, vssEcho->alpha))
                    {
                        ...
                        //	cout << "Current Echo and ready count is "<< it->second.getEchoMsgCnt()<<"
                        //"<<it->second.getReadyMsgCnt()<<endl;
                        //	cout << "Threshold = " << echo_threshold << endl;
                        if ((it->second.getEchoMsgCnt() == echo_threshold)
                            && (it->second.getReadyMsgCnt() < sysparams.get_t() + 1))
                        {
                            ...
                            msgLog << "============================================" << endl;
                            msgLog << "*ENOUGH VSS_ECHO message for " << vssEcho->dealer << " at " << now.tv_sec << "."
                            ...
                            unsigned int                   index =
                                0; // Note that starting with zero as first the value is an evaluation at zero
                            for (iter = activeNodes.begin(); iter != activeNodes.end(); ++iter)
                            {
                                // if (*iter != selfID){
                                ++index;
                                ...
                                gettimeofday(&now, NULL);
                                msgLog << "VSS_READY " << vssReady.get_ID() << " for " << vssReady.dealer
                                       << " SENT from " << selfID << " to " << *iter << " at " << now.tv_sec << "."
                                       << setw(6) << now.tv_usec << " standard 1" << endl;
                            }
                        }
                    }
                    else
                        cerr << "Error at " << selfID << " with the VSSEcho message received from " << buddyID
                             << " for " << vssEcho->dealer << endl;
                }
            }
            break;
            case VSS_READY:
            {
                ...
                gettimeofday(&now, NULL);
                msgLog << "VSS_READY " << vssReady->get_ID() << " for " << vssReady->dealer << " RECEIVED from "
                       << buddyID << " to " << selfID << " at " << now.tv_sec << "." << setw(6) << now.tv_usec << endl;
                ...
                else if (ph < vssReady->ph)
                {
                    ...
                    // Do it when I make decision about the recovery set B
                         ...
                         // set continue
                         ((nodeState != AGREEMENT_COMPLETED)
                          || (find(DecidedVSSs.begin(), DecidedVSSs.end(), vssReady->dealer) != DecidedVSSs.end())))
                {
                    if (vssReady->C.verifyPoint(sysparams, buddyID, selfID, vssReady->alpha))
                    {
                        ...
                        map<VSSReadyMessage, map<NodeID, string>>::iterator ready_it =
                            vssReadyMsg.find(vssReady_SignRemoved);
                        if (ready_it == vssReadyMsg.end())
                        { // No entry for this dealer
                            ...
                            && (it->second.getReadyMsgCnt() == sysparams.get_t() + 1))
                        {
                            ...
                            msgLog << endl << endl << endl;
                            msgLog << "============================================" << endl;
                            msgLog << "*1ENOUGH VSS_READY message for " << selfID << " at " << now.tv_sec << "."
                                   << setw(6) << now.tv_usec << endl;
                        ...
                        else if (it->second.getReadyMsgCnt() == sysparams.get_n() - sysparams.get_t() - sysparams.get_f())
                        {
                            ...
                            msgLog << endl << endl << endl;
                            msgLog << "*============================================" << endl;
                            msgLog << "*2ENOUGH VSS_READY message for " << selfID << " at " << now.tv_sec << "."
                                   << setw(6) << now.tv_usec << endl;
                            msgLog << "*Threshold : Ready = "
                                   << sysparams.get_n() - sysparams.get_t() - sysparams.get_f() << endl;
                            msgLog << "*Received Ready Message Num = " << it->second.getReadyMsgCnt() << endl;
                            ...
                            if (nodeState != AGREEMENT_COMPLETED)
                            {
                                if (selfID != buddyset.get_leader() && nodeState != AGREEMENT_STARTED)
                                {
                                    ...
                                    gettimeofday(&now, NULL);
                                    msgLog << "VSS_SHARED " << vssShared.get_ID() << " for " << vssShared.dealer
                                           << " SENT from " << selfID << " to " << buddyset.get_leader() << " at "
                                           << now.tv_sec << "." << setw(6) << now.tv_usec << " standard 1" << endl;
                                }
                                // I am not erasing entries from C here to avoid false SEND messages for (it->first =
                                // vss->dealer) later
                    ...
                    {
                        msgLog << "Invalid VSSReady Message received at " << selfID << " from " << buddyID << " for "
                               << vssReady->dealer << endl;
                    }
                }
            }
            break;
            case VSS_HELP:
            {
                ...
                gettimeofday(&now, NULL);
                msgLog << "VSS_HELP * for * RECEIVED from " << buddyID << " to " << selfID << " at " << now.tv_sec
                       << "." << setw(6) << now.tv_usec << endl;
                // TODO
                buddy->help(msgLog);
            }
            ...
            case VSS_SHARED:
            {
                ...
                // cerr << "VSS_SHARED for " << vssShared->dealer << " RECEIVED from " << buddyID << " to " << selfID <<
                // endl;
                gettimeofday(&now, NULL);
                msgLog << "VSS_SHARED " << vssShared->get_ID() << " for " << vssShared->dealer << " RECEIVED from "
                       << buddyID << " to " << selfID << " at " << now.tv_sec << "." << setw(6) << now.tv_usec << endl;
                if (selfID == buddyset.get_leader())
                {
                }
            ...
            case DKG_SEND:
            {
                ...
                gettimeofday(&now, NULL);
                msgLog << "DKG_SEND " << dkgSend->get_ID() << " for * RECEIVED from " << buddyID << " to " << selfID
                ...
                else if ((nodeState != AGREEMENT_COMPLETED) && (nodeState != DKG_COMPLETED))
                {
                    ...
                    else if ((dkgSend->msgValid) && (nodeState != LEADER_CHANGE_STARTED))
                    {
                        ...
                        if (dkgReadyValidityMsg.DecidedVSSs.empty() || (dkgReadyValidityMsg.DecidedVSSs == Q))
                        {
                            ...
                            // or no leader sent during last leader, so accept the valid set from the leader and send
                            // Echo
                            vector<NodeID>::iterator iter; // For the active nodes list
                            for (iter = activeNodes.begin(); iter != activeNodes.end(); ++iter)
                            {
                                ...
                                gettimeofday(&now, NULL);
                                msgLog << "DKG_ECHO " << dkgEcho.get_ID() << " for " << dkgEcho.leader << " SENT from "
                                       << selfID << " to " << *iter << " at " << now.tv_sec << "." << setw(6)
                                       << now.tv_usec << " standard 1" << endl;
                            }
                        }
            ...
            case DKG_ECHO:
            {
                ...
                gettimeofday(&now, NULL);
                msgLog << "DKG_ECHO " << dkgEcho->get_ID() << " for " << dkgEcho->leader << " RECEIVED from " << buddyID
                       << " to " << selfID << " at " << now.tv_sec << "." << setw(6) << now.tv_usec << endl;
                if (ph > dkgEcho->ph)
                {
                    // TODO: Actual system need not to display the following messages.
                ...
                else if ((dkgEcho->msgValid) && (nodeState != AGREEMENT_COMPLETED) && (nodeState != DKG_COMPLETED))
                {
                    ...
                    map<DKGEchoOrReadyMessage, map<NodeID, string>>::iterator echo_it =
                        dkgEchoOrReadyMsgReceived.find(dkgEcho_SignRemoved);
                    ...
                    map<DKGEchoOrReadyMessage, map<NodeID, string>>::iterator ready_it =
                        dkgEchoOrReadyMsgReceived.find(dkgReady_SignRemoved);
                    ...
                    if (((NodeIDSize)echo_it->second.size() == echo_threshold) && (dkgReadyCnt < sysparams.get_t() + 1))
                    {
                        ...
                        msgLog << endl << endl << endl;
                        msgLog << "============================================" << endl;
                        msgLog << "*ENOUGH DKG_ECHO message for " << selfID << " at " << now.tv_sec << "." << setw(6)
                               << now.tv_usec << endl;
                        msgLog << "*Threshold : Echo = " << echo_threshold << endl;
                        ...
                        for (iter = activeNodes.begin(); iter != activeNodes.end(); ++iter)
                        {
                            ...
                            gettimeofday(&now, NULL);
                            msgLog << "DKG_READY " << dkgReady.get_ID() << " for " << dkgReady.leader << " SENT from "
                                   << selfID << " to " << *iter << " at " << now.tv_sec << "." << setw(6) << now.tv_usec
                                   << " standard 1" << endl;
                        }
                    }
                }
                ...
                {
                    gettimeofday(&now, NULL);
                    msgLog << "Invalid DKGEcho for " << dkgEcho->leader << " from " << buddyID << " to " << selfID
                           << " RECEIVED at " << now.tv_sec << "." << setw(6) << now.tv_usec << "\n";
                }
            }
            break;
            case DKG_READY:
            {
                ...
                gettimeofday(&now, NULL);
                msgLog << "DKG_READY " << dkgReady->get_ID() << " for " << dkgReady->leader << " RECEIVED from "
                       << buddyID << " to " << selfID << " at " << now.tv_sec << "." << setw(6) << now.tv_usec << endl;
                ...
                else if ((dkgReady->msgValid) && (nodeState != AGREEMENT_COMPLETED) && (nodeState != DKG_COMPLETED))
                {
                    ...
                    if ((dkgEchoCnt < echo_threshold) && ((NodeIDSize)ready_it->second.size() == sysparams.get_t() + 1))
                    {
                        ...
                        msgLog << endl << endl << endl;
                        msgLog << "============================================" << endl;
                        msgLog << "*1ENOUGH DKG_READY message for " << selfID << " at " << now.tv_sec << "." << setw(6)
                               << now.tv_usec << endl;
                        msgLog << "*Threshold : Ready = " << sysparams.get_t() + 1 << endl;
                        ...
                        for (iter = activeNodes.begin(); iter != activeNodes.end(); ++iter)
                        {
                            ...
                            gettimeofday(&now, NULL);
                            msgLog << "DKG_READY " << dkgReadySent.get_ID() << " for " << dkgReadySent.leader
                                   << " SENT from " << selfID << " to " << *iter << " at " << now.tv_sec << "."
                                   << setw(6) << now.tv_usec << " standard 1" << endl;
                        }
                    }
                    else if ((NodeIDSize)ready_it->second.size()
                             == sysparams.get_n() - sysparams.get_t() - sysparams.get_f())
                    {
                        ...
                        msgLog << endl << endl << endl;
                        msgLog << "============================================" << endl;
                        msgLog << "*2ENOUGH DKG_READY message for " << selfID << " at " << now.tv_sec << "." << setw(6)
                               << now.tv_usec << endl;
                        msgLog << "*Threshold : Ready = " << sysparams.get_n() - sysparams.get_t() - sysparams.get_f()
                               << endl;
                        msgLog << "*Received Ready Message Num = " << (NodeIDSize)ready_it->second.size() << endl;
                        msgLog << "============================================" << endl << endl;
                        ...
                        // cout << "COMPLETE!!! for node " << selfID << " at " << now.tv_sec << "." << setw(6) <<
                        // now.tv_usec << endl;
                        completeDKG();
                    }
                }
            ...
            {
                gettimeofday(&now, NULL);
                msgLog << "DKG_HELP * for * RECEIVED from " << buddyID << " to " << selfID << " at " << now.tv_sec
                       << "." << setw(6) << now.tv_usec << endl;
            ...
            case LEADER_CHANGE:
            {
                ...
                gettimeofday(&now, NULL);
                msgLog << "LEADER_CHANGE " << leaderChange->get_ID() << " for " << leaderChange->nextLeader
                       << " RECEIVED from " << buddyID << " to " << selfID << " at " << now.tv_sec << "." << setw(6)
                       << now.tv_usec << endl;
                if (ph > leaderChange->ph)
                {
                    // TODO: Actual system need not to display the following messages.
                    cerr << "Phase for LeaderChange " << leaderChange->ph << " is older than the current phase " << ph
                         << endl;
                }
                ...
                else if ((leaderChange->msgValid) && (nodeState != DKG_COMPLETED) && (nodeState != AGREEMENT_COMPLETED))
                {
                    ...
                    {
                        if ((leaderChange->nextLeader + sysparams.get_n() - buddyset.get_leader()) % sysparams.get_n()
                            > sysparams.get_t() + sysparams.get_f())
                        {
                        ...
                        map<LeaderChangeMessage, map<NodeID, string>>::iterator it =
                            leaderChangeMsg.find(leaderChange_SignRemoved);
                        ...
                        for (NodeID index = 0; index < activeNodes.size(); ++index)
                            if (activeNodes[index] == buddyset.get_leader())
                            {
                                index_L = index;
                                break;
                            }
                        for (NodeID index = 0; index < activeNodes.size(); ++index)
                            if (activeNodes[index] == leaderChange->nextLeader)
                            {
                                index_received = index;
                                break;
                            }
                        for (NodeID index = 0; index < activeNodes.size(); ++index)
                            if (activeNodes[index] == nextSmallestLeader)
                            {
                                index_smallest = index;
                                break;
                            }
                        // cerr<<"index_L, index_received, index_smallest resp. are "<<index_L <<" "<< index_received<<"
                        // "<< index_smallest<<endl;
                        ...
                        { // Add Q, R/M to Q_cap R_cap
                            ...
                            for (it_received = leaderChange->vssReadyMsg.begin();
                                 it_received != leaderChange->vssReadyMsg.end();
                                 ++it_received)
                            {
                                // Copy to  vssReadyMsg
                                ...
                                if (NodeIDSize(vssReadyMsgSelected.size()) < sysparams.get_t() + 1)
                                {
                                    ...
                                    if (it == vssReadyMsgSelected.end()) // No entry for this dealer in local
                                                                         // vssReadyMsg
                                        it = vssReadyMsgSelected
                                                 .insert(make_pair(it_received->first, it_received->second))
                                                 .first;
                                    else // Entry for this dealer exists. add the received ready messages
                                        it->second.insert(it_received->second.begin(), it_received->second.end());
                                }
                                 ...
                                 == sysparams.get_n() - sysparams.get_t() - sysparams.get_f())
                        {
                            ...
                            gettimeofday(&now, NULL);
                            msgLog << "NEW_LEADER * : " << leaderChange->nextLeader << " NL from * to * at "
                    ...
                    {
                        // DKG is just started. This is a message for the leader to collect required leaderchange
                        // certificate
                        // Node
            ...
            {
                cerr << "PUBLIC_KEY_EXCHANGE message from" << buddyID << " to " << selfID << endl;
            ...
            {
                cerr << "BLS_SIGNATURE_REQUEST message from" << buddyID << " to " << selfID << endl;
                ...
                {
                    // TODO: Actual system need not to display the following messages.
                    cerr << "Phase for BLS_SIGNATURE_REQUEST" << signatureRequest->ph
                         << " is older than the current phase " << ph << endl;
                }
                else
                {
            ...
            case WRONG_BLS_SIGNATURES:
            {
                ...
                {
                    // TODO: Actual system need not to display the following messages.
                    cerr << "Phase for WRONG_BLS_SIGNATURES" << wrongSignatures->ph
                         << " is older than the current phase " << ph << endl;
                }
                else
                {
                    ...
                    for (map<NodeID, G1>::iterator iter = wrongSignatures->signatures.begin();
                         iter != wrongSignatures->signatures.end();
                         ++iter)
                    {
        ...
        case TIMER:
        {
            ...
            case TIMER_MSG_LEADER_CHANGE:
            {
                ...
                gettimeofday(&now, NULL);
                msgLog << "TIMEOUT " << tm->get_ID() << " for " << buddyset.get_leader() << " RECEIVED from " << selfID
                       << " to " << buddyset.get_next_leader() << " at " << now.tv_sec << "." << setw(6) << now.tv_usec
                       << endl;
                sendLeaderChangeMessage(buddyset.get_next_leader());
                //}
            }
...
void
Node::hybridVSSInit(const Zr & secret)
{
    // const Pairing& e = sysparams.get_Pairing();
    ...
    gettimeofday(&now, NULL);
    msgLog << "COMPUTE_BI * for * RS from " << selfID << " to * at " << now.tv_sec << "." << setw(6) << now.tv_usec
           << endl;
    BiPolynomial fxy(sysparams, t, secret);

    gettimeofday(&now, NULL);
    msgLog << "COMPUTE_CM * for * RS from " << selfID << " to * at " << now.tv_sec << "." << setw(6) << now.tv_usec
           << endl;
    ...
    for (iter = activeNodes.begin(); iter != activeNodes.end(); ++iter)
    {
        ...
        gettimeofday(&now, NULL);
        msgLog << "VSS_SEND " << vssSend.get_ID() << " for "
               << "* SENT from " << selfID << " to " << *iter << " at " << now.tv_sec << "." << setw(6) << now.tv_usec
               << " standard 1" << endl;
    }
    //  cerr << endl << endl;
}
...
Node::startAgreement()
{
    ...
    timeval now;
    if (selfID == buddyset.get_leader() && selfID > non_responsive_leader_number)
    { // The node is the current leader
        ...
        msgLog << "*ENOUGH VSS_READY message for " << selfID << " at " << now.tv_sec << "." << setw(6) << now.tv_usec
               << endl;
        msgLog << "*Threshold : Ready = " << sysparams.get_t() + 1 << endl;
        // msgLog << "Received Ready Message Num = " << it->second.getReadyMsgCnt() << endl;
        msgLog << "*Ready messages are sending out..." << endl;
        ...
        { // DKGEcho or DKGReady are from the previous leader are used
            for (vector<NodeID>::iterator iter = activeNodes.begin(); iter != activeNodes.end(); ++iter)
            {
                // if (*iter != selfID){
                DKGSendMessage dkgSend(ph,
                                       it_leadchg->first,
                                       it_leadchg->second,
                                       (NetworkMessageType)dkgReadyValidityMsg.strMsg[0],
                                       dkgReadyValidityMsg,
                                       dkgReadyValidityMsgDSAs);
                buddyset.send_message(*iter, dkgSend);
                gettimeofday(&now, NULL);
                msgLog << "DKG_SEND " << dkgSend.get_ID() << " for "
                       << "* SENT from " << selfID << " to " << *iter << " at " << now.tv_sec << "." << setw(6)
                       << now.tv_usec << " standard 1" << endl;
            }
        }
    ...
    { // The node is not the current leader
        ...
        gettimeofday(&now, NULL);
        timeoutLog << endl
                   << "time_diff from node# " << selfID << " is " << time_diff << " ms @ " << now.tv_sec << "."
                   << setw(6) << now.tv_usec << endl;
        leaderChangeTimerID = Timer::new_timer(new LeaderChangeTimerMessage(buddyset.get_next_leader(), ph), time_diff);
...
Node::completeDKG()
{
    ...
    if (!VSSsCompleted)
    {
        cerr << "All VSS not yet complete\n";
        return;
    } // All required VSSs are not yet completed
    ...
    gettimeofday(&now, NULL);
    msgLog << "DKG_COMPLETE * for " << buddyset.get_leader() << " RS from " << selfID << " to * at " << now.tv_sec
           << "." << setw(6) << now.tv_usec << " :)" << endl;
    cout << "DKG_COMPLETE * for " << buddyset.get_leader() << " RS from " << selfID << " to * at " << now.tv_sec << "."
         << setw(6) << now.tv_usec << " :)" << endl;
...
void
Node::sendLeaderChangeMessage(NodeID nextLeader)
{
    timeval now;
    ...
    if ((nodeState != LEADER_CHANGE_STARTED) && (nodeState != DKG_COMPLETED))
    {
        ...
        if (dkgReadyValidityMsgDSAs.size())
        {
            ...
            {
                // if (*iter != selfID){
                LeaderChangeMessage leadChg = LeaderChangeMessage(buddyset,
                                                                  ph,
                                                                  nextLeader,
                                                                  (NetworkMessageType)dkgReadyValidityMsg.strMsg[0],
                                                                  dkgReadyValidityMsg,
                ...
                gettimeofday(&now, NULL);
                msgLog << "LEADER_CHANGE " << leadChg.get_ID() << " for " << nextLeader << " SENT from " << selfID
                       << " to " << *iter << " at " << now.tv_sec << "." << setw(6) << now.tv_usec << " standard 1"
                       << endl;
            }
        }
        else
...
main(int argc, char ** argv)
{
    ...
    {
        cerr << "Usage: " << argv[0]
             << " portnum certfile keyfile contactlist phase CommitmentType[0/1] non_responsive_leader_number\n";
        exit(1);
    }
