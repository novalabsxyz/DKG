...
G2::G2(const Pairing & e, const unsigned char * data, unsigned short len, bool compressed, unsigned short base)
    : G(e)
{
    if (elementPresent)
    {
...
unsigned short
G2::getElementSize(bool compressed) const
{
    if (!elementPresent)
        throw UndefinedElementException();
    else if (compressed)
        return (unsigned short)element_length_in_bytes_compressed(*(element_t *)&g);
    else
...
string
G2::toString(bool compressed) const
{
    string str;
    if (compressed)
    {
