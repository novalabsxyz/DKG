...
BiPolynomial &
BiPolynomial::merge(const BiPolynomial & rhs, void (*mfunc)(Zr &, const Zr))
{
...
// element_t coeff
const Zr
BiPolynomial::getCoeff(unsigned int i, unsigned int j) const
{
    if (i < coeffs.size())
        if (j < coeffs[i].size())
            return coeffs[i][j];
...
const Polynomial
BiPolynomial::apply(const Zr & x) const
{
    Polynomial result;
...
void
BiPolynomial::dump(FILE * f, char * label, unsigned short base) const
{
    if (label)
        fprintf(f, "%s: ", label);
    fprintf(f, "[\n ");
