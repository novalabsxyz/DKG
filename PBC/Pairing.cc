...
// Apply Pairing
const GT
Pairing::operator()(const G1 & p, const G2 & q) const
{
    if (pairingPresent)
        if (p.isElementPresent() && q.isElementPresent())
        {
            ...
            pairing_apply(gt, *(element_t *)&p.getElement(), *(element_t *)&q.getElement(), *(pairing_t *)&e);
            GT ans;
            ans.setElement(gt);
            element_clear(gt);
            return ans;
...
// Symmetric Pairings
const GT
Pairing::operator()(const G1 & p, const G1 & q) const
{
    if (pairingPresent)
        if (p.isElementPresent() && q.isElementPresent())
            if (isSymmetric())
            {
                ...
                throw NonsymmetricPairingException();
        else
            throw UndefinedElementException();
    else
        throw UndefinedPairingException();
...
const GT
Pairing::operator()(const G2 & p, const G2 & q) const
{
    if (pairingPresent)
        if (p.isElementPresent() && q.isElementPresent())
            if (isSymmetric())
...
// Apply Pairing
const GT
Pairing::apply(const G1 & p, const G2 & q) const
{
    return (*this)(p, q);
}
// Symmetric Pairings
const GT
Pairing::apply(const G1 & p, const G1 & q) const
{
    return (*this)(p, q);
}
const GT
Pairing::apply(const G2 & p, const G2 & q) const
{
    return (*this)(p, q);
}
...
size_t
Pairing::getElementSize(PairingElementType type, bool compressed) const
{
    if (pairingPresent)
    {
        switch (type)
