...
class GT : public G
{
  public:
    ...
    GT(const Pairing & e, const unsigned char * data, unsigned short len, unsigned short base = 16);

    // Create an element from hash
    GT(const Pairing & e, const void * data, unsigned short len);
    ...
    GT & operator^=(const Zr & exp)
    {
        return (GT &)G::operator^=(exp);
    }
