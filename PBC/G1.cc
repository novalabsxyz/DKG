...
G1::G1(const Pairing & e, const unsigned char * data, unsigned short len, bool compressed, unsigned short base)
    : G(e)
{
    if (elementPresent)
    {
...
unsigned short
G1::getElementSize(bool compressed) const
{
    if (!elementPresent)
...
string
G1::toString(bool compressed) const
{
    string str;
    if (compressed)
    {
