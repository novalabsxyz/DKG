...
Commitment::Commitment(const SystemParam & sys, const vector<NodeID> & activeNodes, CommitmentType type)
    : hashedVector(sys, activeNodes)
    , matrix(sys)
    , type(type)
{
}

Commitment::Commitment(const SystemParam &    sys,
                       const vector<NodeID> & activeNodes,
                       const BiPolynomial &   fxy,
                       CommitmentType         type)
    : hashedVector(sys, activeNodes, fxy)
    , matrix(sys, fxy)
    , type(type)
{
}
...
Commitment::Commitment(const SystemParam & sys, const unsigned char *& buf, size_t & len)
{
    ...
    read_byte(buf, len, commType);
    type = (CommitmentType)commType;
    if (type == Feldman_Matrix)
    {
...
string
Commitment::toString(bool includeSubshares) const
{
    string str;
    write_byte(str, type);
    if (type == Feldman_Matrix)
...
bool
Commitment::operator==(const Commitment & rhs) const
{
...
bool
Commitment::verifyPoly(const SystemParam & sys, NodeID verifierID, const Polynomial & poly)
{
    if (type == Feldman_Matrix)
...
const G1
Commitment::publicKeyShare(const SystemParam & sys, NodeID nodeID) const
{
    if (type == Feldman_Matrix)
...
bool
Commitment::verifyPoint(const SystemParam & sys, NodeID senderID, NodeID verifierID, const Zr & point) const
{
    if (type == Feldman_Matrix)
...
const vector<Zr>
Commitment::interpolate(const SystemParam & sys, bool EchoOrReady, const vector<NodeID> & activeList) const
{
    ...
    {
        if (A_C.find(*ID_it) == A_C.end())
        { // Haven't received share from *(ID_it)
            alpha             = Zr(sys.get_Pairing(), (long)*ID_it);
            vector<Zr> coeffs = lagrange_coeffs(indices, alpha);
            subshares.push_back(lagrange_apply(coeffs, evals));
        }
        else
            subshares.push_back(A_C.find(*(ID_it))->second);
    }
    return subshares;
}
void
Commitment::dump(FILE * f, unsigned int indent) const
{
    if (type == Feldman_Matrix)
    {
    ...
    fprintf(f, "%*s  echo message count = %zu\n", indent, "", A_Echo.size());
    fprintf(f, "%*s  ready message count = %zu\n", indent, "", A_Ready.size());
}
