...
Zr::Zr(const Pairing & e, const unsigned char * data, unsigned short len, unsigned short base)
{
    elementPresent = e.isPairingPresent();
    if (elementPresent)
    {
...
bool
Zr::operator==(const Zr & rhs) const
{
    if (elementPresent && rhs.isElementPresent())
    {
        return !(element_cmp(*(element_t *)&r, *(element_t *)&rhs.getElement()));
    }
    else
        throw UndefinedElementException();
}
...
bool
Zr::isIdentity(bool additive) const
{
    if (elementPresent)
    {
        if (additive)
...
const Zr
Zr::inverse(bool additive) const
{
    if (elementPresent)
    {
        Zr s(*this);
        if (additive)
            element_neg(*(element_t *)&s.getElement(), *(element_t *)&s.getElement());
        else
            element_invert(*(element_t *)&s.getElement(), *(element_t *)&s.getElement());
        return s;
    }
    else
        throw UndefinedElementException();
...
Zr::square() const
{
    ...
    {
        Zr s(*this);
        element_square(*(element_t *)&s.getElement(), *(element_t *)&s.getElement());
        return s;
    }
    else
        throw UndefinedElementException();
...
void
Zr::setElement(const element_t & el)
{
    nullify();
...
unsigned short
Zr::getElementSize() const
{
    if (elementPresent)
        return (unsigned short)element_length_in_bytes(*(element_t *)&r) + 1;
...
Zr::toString() const
{
    ...
    if (elementPresent)
    {
        ...
        element_to_bytes(data, *(element_t *)&r);
...
void
Zr::dump(FILE * f, const char * label, unsigned short base) const
{
    if (label)
        fprintf(f, "%s: ", label);
    // fprintf(f, "[ ");
