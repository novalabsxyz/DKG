...
NetworkMessage *
NetworkMessage::read_message(SystemType systemtype, const Buddy * buddy)
{
    ...
    int g_recv_ID = (msgStr[0] << 24) | (((msgStr[1]) << 16) & 0x00ffffff) | (((msgStr[2]) << 8) & 0x0000ffff)
                    | (msgStr[3] & 0x000000ff);
...
PingNetworkMessage::PingNetworkMessage(const BuddySet & buddyset, unsigned int t)
    : t(t)
{
    string msgbody;
    ...
    : NetworkMessage(str)
{
    ...
    strMsg   = toString();
    msgValid = read_sig(buddy,
                        bodyptr,
                        bodylen,
                        (const unsigned char *)strMsg.data(),
...
string
PingNetworkMessage::toString() const
{
    string msgStr;
    write_ui(msgStr, t);
...
VSSReadyMessage::VSSReadyMessage(const BuddySet &   buddyset,
                                 NodeID             dealer,
                                 Phase              ph,
                                 const Commitment & C,
                                 const Zr &         alpha,
...
string
VSSReadyMessage::toString() const
{
    string strMsg;
...
VSSSharedMessage::VSSSharedMessage(Phase                   ph,
                                   NodeID                  dealer,
                                   const VSSReadyMessage & readyMsg,
    ...
    : NetworkMessage(str)
{
    ...
    if (size < 2 * buddy->get_param().get_t() + 1)
    {
        msgValid = false;
        return;
    }
    ...
    {
        NodeID sender;
        read_us(bodyptr, bodylen, sender);
...
*/
DKGSendMessage::DKGSendMessage(Phase                       ph,
                               const LeaderChangeMessage & leadChgMsg,
                               const map<NodeID, string> & leadChgMsgDSAs,
                               const map<VSSReadyMessage, map<NodeID, string>, VSSReadyMessageCmp> & vssReadyMsg)
    : ph(ph)
    , leadChgMsg(leadChgMsg)
    , leadChgMsgDSAs(leadChgMsgDSAs)
    , msgType(VSS_READY)
    , vssReadyMsg(vssReadyMsg)
{
    string body;
...
DKGSendMessage::DKGSendMessage(Phase                       ph,
                               ...
                               const DKGEchoOrReadyMessage & dkgEchoOrReadyMsg,
                               const map<NodeID, string> & dkgEchoOrReadyMsgDSAs)
    : ph(ph)
    , leadChgMsg(leadChgMsg)
    , leadChgMsgDSAs(leadChgMsgDSAs)
    , msgType(msgType)
    , dkgEchoOrReadyMsg(dkgEchoOrReadyMsg)
    ...
    : NetworkMessage(str)
{
    ...
    unsigned char charLeaderChangePresent;
    read_byte(bodyptr, bodylen, charLeaderChangePresent);
    ...
    if (isLeaderChangePresent)
    {
        ...
        const unsigned char * signend = bodyptr;
        NodeIDSize            size1d;
        read_us(bodyptr, bodylen, size1d);
        if (size1d < 2 * t + 1)
        {
            cerr << "Not enough lc signs\n";
            msgValid = false;
            return;
        }
        for (NodeIDSize j = 0; j < size1d; ++j)
        {
            ...
            if (!msgValid)
            {
                cerr << "Invalid lc Signature from sender " << sender << " "
                     << buddy->find_other_buddy(sender)->get_id() << endl;
                return;
    ...
    unsigned char  type;
    read_byte(bodyptr, bodylen, type);
    ...
    case VSS_READY:
    {
        ...
        for (unsigned short i = 0; i < size2d; ++i)
        {
            ...
            bodyptr += length;
            const unsigned char * signend = bodyptr;
            unsigned short        size1d;
            read_us(bodyptr, bodylen, size1d);
            if (size1d < 2 * t + 1)
            {
                cerr << "Not enough VSS signs\n";
                msgValid = false;
                return;
            }
            ...
            for (unsigned short j = 0; j < size1d; ++j)
            {
                ...
                if (!msgValid)
                {
                    cerr << "Invalid vss Signature for j=" << j << endl;
                    return;
                } // Invalid Signature Signature
                string DSA = str.substr(str.size() - bodylen - buddy->sig_size(), buddy->sig_size());
    ...
    case DKG_READY:
    {
        ...
        if (size1d < t + 1)
        {
            cerr << "Not enough DKGReady signs\n";
            msgValid = false;
            return;
        }
        for (unsigned short j = 0; j < size1d; ++j)
...
DKGEchoOrReadyMessage::DKGEchoOrReadyMessage(const BuddySet &    buddyset,
                                             NetworkMessageType  type,
                                             NodeID              leader,
                                             Phase               ph,
                                             const set<NodeID> & DecidedVSSs,
                                             bool                includeSignature)
    : leader(leader)
    , ph(ph)
    , DecidedVSSs(DecidedVSSs)
{
    string body;
    // size_t signstart = body.size();
...
string
DKGEchoOrReadyMessage::toString(NetworkMessageType type) const
{
    string body;
...
DKGHelpMessage::DKGHelpMessage(const BuddySet & buddyset, NodeID leader, Phase ph)
    : leader(leader)
    , ph(ph)
{
    string body;
...
DKGHelpMessage::DKGHelpMessage(const Buddy * buddy, const string & str, int g_recv_ID)
    : NetworkMessage(str)
{
    const unsigned char * bodyptr = (const unsigned char *)str.data() + headerLength;
    size_t                bodylen = str.size() - headerLength;
...
LeaderChangeMessage::LeaderChangeMessage(const BuddySet & buddyset,
                                         Phase            ph,
                                         NodeID           nextLeader,
                                         const map<VSSReadyMessage, map<NodeID, string>, VSSReadyMessageCmp> & vssReadyMsg,
                                         bool includeSignature)
...
LeaderChangeMessage::LeaderChangeMessage(const BuddySet &              buddyset,
                                         ...
                                         NetworkMessageType            msgType,
                                         const DKGEchoOrReadyMessage & dkgEchoOrReadyMsg,
                                         const map<NodeID, string> & DSAs,
                                         bool includeSignature)
    : ph(ph)
    , nextLeader(nextLeader)
    , msgType(msgType)
    , dkgEchoOrReadyMsg(dkgEchoOrReadyMsg)
    , dkgEchoOrReadyMsgDSAs(DSAs)
{
    string body;
    // size_t signstart = body.size();
    write_ui(body, ph);
    ...
    : NetworkMessage(str)
{
    ...
    read_byte(bodyptr, bodylen, typeChar);
    ...
    else if ((msgType == DKG_ECHO) || (msgType == DKG_READY))
    {
        ...
        read_ui(bodyptr, bodylen, length);
        if (length)
            dkgEchoOrReadyMsg = DKGEchoOrReadyMessage(buddy, NET_MSG_NONE, str.substr(str.length() - bodylen, length));
        bodylen -= length;
        bodyptr += length;
        ...
        const unsigned char * end =
            (const unsigned char *)dkgEchoOrReadyMsg.strMsg.data() + dkgEchoOrReadyMsg.strMsg.length();
        read_us(bodyptr, bodylen, size);
...
string
LeaderChangeMessage::toString() const
{
    string body;
    write_ui(body, ph);
    write_us(body, nextLeader);
...
BLSSignatureRequestMessage::BLSSignatureRequestMessage(const BuddySet & buddyset,
                                                       Phase            ph,
                                                       const string &   msg,
                                                       const G1 &       signature)
    : ph(ph)
...
BLSSignatureResponseMessage::BLSSignatureResponseMessage(const BuddySet & buddyset,
                                                         Phase            ph,
                                                         const G1 &       msgHash,
                                                         const G1 &       signatureShare)
    : ph(ph)
    , msgHash(msgHash)
...
BLSSignatureResponseMessage::BLSSignatureResponseMessage(const Buddy * buddy, const string & str, int g_recv_ID)
    : NetworkMessage(str)
{
    const unsigned char * bodyptr = (const unsigned char *)str.data() + headerLength;
    size_t                bodylen = str.size() - headerLength;
...
WrongBLSSignaturesMessage::WrongBLSSignaturesMessage(const BuddySet & buddyset,
                                                     Phase            ph,
                                                     const G1 &       msgHash,
                                                     const map<NodeID, G1> & signatures)
    : ph(ph)
    , msgHash(msgHash)
...
VerifiedBLSSignaturesMessage::VerifiedBLSSignaturesMessage(const BuddySet & buddyset,
                                                           Phase            ph,
                                                           const G1 &       msgHash,
                                                           const map<NodeID, G1> & signatures)
    : ph(ph)
