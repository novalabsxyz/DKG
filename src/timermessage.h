...
typedef enum { TIMER_MSG_NONE, TIMER_MSG_LEADER_CHANGE, TIMER_MSG_PHASE_CHANGE } TimerMessageType;
...
class TimerMessage : public Message
{
  public:
    ...
    void set_id(TimerID id)
    {
        this->id = id;
    }
    TimerMessageType get_type() const
    {
        return type;
    }
  ...
  protected:
    ...
    TimerID          id;
    TimerMessage(TimerMessageType type)
        : type(type)
    {
        message_class = TIMER;
    }
...
class LeaderChangeTimerMessage : public TimerMessage
{
  public:
    LeaderChangeTimerMessage(NodeID leader, Phase ph)
        : TimerMessage(TIMER_MSG_LEADER_CHANGE)
        , leader(leader)
        , ph(ph)
    {
    }
...
class PhaseChangeTimerMessage : public TimerMessage
{
  public:
    PhaseChangeTimerMessage(Phase nextPh)
        : TimerMessage(TIMER_MSG_PHASE_CHANGE)
        , nextPh(nextPh)
    {
    }
    Phase nextPh;
