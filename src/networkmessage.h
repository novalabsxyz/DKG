...
typedef enum {
    ...
    RECONSTRUCT_SHARE,
    PUBLIC_KEY_EXCHANGE,
    BLS_SIGNATURE_REQUEST,
    BLS_SIGNATURE_RESPONSE,
    WRONG_BLS_SIGNATURES,
    VERIFIED_BLS_SIGNATURES
} NetworkMessageType;
...
class NetworkMessage : public Message
{
  public:
    ...
    const string & get_netMsgStr() const
    {
        return netMsgStr;
    }
...
class VSSReadyMessage : public NetworkMessage
{
  public:
    ...
    VSSReadyMessage(const BuddySet &   buddyset,
                    NodeID             dealer,
                    Phase              ph,
...
struct VSSReadyMessageCmp
{
    bool operator()(VSSReadyMessage m1, VSSReadyMessage m2) const
    {
        ...
        if (m1.ph > m2.ph)
            return 1;
        if (m1.dealer < m2.dealer)
            return 0;
        if (m1.dealer > m2.dealer)
            return 1;
        if (m1.C.toString(false) < m2.C.toString(false))
            return 0;
        if (m1.C.toString(false) > m2.C.toString(false))
            return 1;
        return 0;
...
{
  public:
    VSSSharedMessage(Phase ph, NodeID dealer, const VSSReadyMessage & readyMsg, const map<NodeID, string> & msgDSAs);
...
class DKGEchoOrReadyMessage : public NetworkMessage
{
  public:
    ...
    DKGEchoOrReadyMessage(const BuddySet &    buddyset,
...
struct DKGEchoOrReadyMessageCmp
{
    bool operator()(DKGEchoOrReadyMessage m1, DKGEchoOrReadyMessage m2) const
    {
        ...
        if (m1.leader > m2.leader)
            return 1;
        if (m1.DecidedVSSs < m2.DecidedVSSs)
            return 0;
        if (m1.DecidedVSSs > m2.DecidedVSSs)
            return 1;
        return 0;
...
class LeaderChangeMessage : public NetworkMessage
{
  public:
    ...
    LeaderChangeMessage(const BuddySet & buddyset,
                        Phase            ph,
                        NodeID           nextLeader,
...
struct LeaderChangeMessageCmp
{
    bool operator()(LeaderChangeMessage lc1, LeaderChangeMessage lc2) const
    {
        ...
        if (lc1.ph > lc2.ph)
            return 1;
        if (lc1.nextLeader < lc2.nextLeader)
            return 0;
        if (lc1.nextLeader > lc2.nextLeader)
            return 1;
        return 0;
...
{
  public:
    DKGEchoMessage(const BuddySet &    buddyset,
                   NodeID              leader,
                   Phase               ph,
                   const set<NodeID> & DecidedVSSs,
                   bool                includeSignature = true)
...
{
  public:
    DKGReadyMessage(const BuddySet &    buddyset,
                    NodeID              leader,
                    Phase               ph,
                    const set<NodeID> & DecidedVSSs,
                    bool                includeSignature = true)
...
{
  public:
    VerifiedBLSSignaturesMessage(const BuddySet & buddyset,
                                 Phase            ph,
                                 const G1 &       msgHash,
