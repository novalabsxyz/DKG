...
class Commitment
{
  ...
  public:
    ...
    Commitment(const SystemParam & sys, const vector<NodeID> & activeNodes, const BiPolynomial & fxy, CommitmentType type);
    ...
    unsigned short getEchoMsgCnt() const
    {
        return (unsigned short)A_Echo.size();
    }
    unsigned short getReadyMsgCnt() const
    {
        return (unsigned short)A_Ready.size();
    }

    bool operator==(const Commitment & rhs) const;

    CommitmentType get_Type() const
    {
        return type;
    }
    const CommitmentMatrix get_Matrix() const
    {
        return matrix;
    }
    const CommitmentVector get_Vector() const
    {
        return hashedVector;
    }
    ...
    {
        hashedVector.setSubshares(sys, values);
    }
