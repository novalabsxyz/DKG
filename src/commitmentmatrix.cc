...
CommitmentMatrix::CommitmentMatrix(const SystemParam & sys, const BiPolynomial & fxy)
{
...
// Deserialize
CommitmentMatrix::CommitmentMatrix(const SystemParam & sys, const unsigned char *& buf, size_t & len)
{
    G1             U = sys.get_U();
    unsigned short rowcnt;
    read_us(buf, len, rowcnt);
    for (unsigned short i = 0; i < rowcnt; ++i)
    {
        vector<G1>     row;
        unsigned short colcnt;
        read_us(buf, len, colcnt);
...
string
CommitmentMatrix::toString() const
{
    string                             returnStr;
...
const G1
CommitmentMatrix::getEntry(unsigned short i, unsigned short j) const
{
    if (i < entries.size())
        if (j < entries[i].size())
            return entries[i][j];
...
bool
CommitmentMatrix::operator==(const CommitmentMatrix & rhs) const
{
    for (unsigned int i = 0; i < entries.size(); ++i)
        for (unsigned int j = 0; j < entries[i].size(); ++j)
        {
            if (!(entries[i][j] == rhs.getEntry(i, j)))
                return false;
        }
    return true;
}
...
bool
CommitmentMatrix::verifyPoly(const SystemParam & sys, NodeID verifierID, const Polynomial & poly) const
{
...
bool
CommitmentMatrix::verifyPoint(const SystemParam & sys, NodeID senderID, NodeID verifierID, const Zr & point) const
{
...
const G1
CommitmentMatrix::publicKeyShare(const SystemParam & sys, NodeID nodeID) const
{
...
void
CommitmentMatrix::dump(FILE * f, unsigned int indent) const
{
    vector<vector<G1>>::const_iterator iter2d;
    vector<G1>::const_iterator         iter1d;
