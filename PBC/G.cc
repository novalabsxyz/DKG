...
G::operator=(const G & rhs)
{
    ...
    if (elementPresent)
    {
        element_init_same_as(g, *(element_t *)&rhs.getElement());
...
G::operator==(const G & rhs) const
{
    if (elementPresent && rhs.isElementPresent())
    {
        return !(element_cmp(*(element_t *)&g, *(element_t *)&rhs.getElement()));
    }
    else
        throw UndefinedElementException();
}
...
G::inverse() const
{
    ...
    {
        G h(*this);
        element_invert(*(element_t *)&h.getElement(), *(element_t *)&h.getElement());
        return h;
    }
    else
        throw UndefinedElementException();
...
G::square() const
{
    ...
    {
        G h(*this);
        element_square(*(element_t *)&h.getElement(), *(element_t *)&h.getElement());
        return h;
    }
    else
        throw UndefinedElementException();
...
void
G::setElement(const element_t & el)
{
    nullify();
...
unsigned short
G::getElementSize() const
{
    if (elementPresent)
        return (unsigned short)element_length_in_bytes(*(element_t *)&g);
    else
        throw UndefinedElementException();
}
...
void
G::dump(FILE * f, const char * label, unsigned short base) const
{
    if (label)
        fprintf(f, "%s: ", label);
    if (elementPresent)
