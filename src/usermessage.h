...
class UserMessage : public Message
{
  public:
    ...
    {
        message_class = USER;
        msgtype       = USER_MSG_NONE;
    }
    UserMessageType get_type() const
    {
        return msgtype;
    }
...
{
  public:
    PingUserMessage(int who)
        : who(who)
    {
        msgtype = USER_MSG_PING;
    }
    int get_who() const
    {
        return who;
...
{
  public:
    ConfirmLeaderMessage()
    {
        msgtype = CONFIRM_LEADER;
    }
};
...
typedef enum { NONE, ID, N, T, U, F, STATE, PHASE, LEADER, COMMITMENT, MYSHARE, ACTIVE_NODES } StateInformationType;
...
{
  public:
    BLSSignatureRequestUserMessage()
    {
        msgtype = SIGN;
    }
    BLSSignatureRequestUserMessage(const string & str)
    {
        msgtype = SIGN;
        msg     = str;
    }
