...
class BuddySet
{
  public:
    ...
    void sign(const unsigned char * data, size_t len, unsigned char * sig) const;
    const map<BuddyID, ContactEntry> & get_buddy_list() const
    {
        return contactlist;
    }
    BuddyID get_my_id() const
    {
        return my_id;
    }
    BuddyID get_leader() const
    {
        return leader;
    }
    ...
    int get_first_msg_type(BuddyID id)
    {
        return first_msg_type[id];
    }
    ...
    {
        for (map<BuddyID, int>::iterator fmtiter = first_msg_type.begin(); fmtiter != first_msg_type.end(); fmtiter++)
        {
            if (fmtiter->first == id)
            {
                return false;
