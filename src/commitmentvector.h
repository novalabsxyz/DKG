...
class CommitmentVector
{
  ...
  public:
    ...
    const G1 getShare(unsigned short i) const
    {
        return shares[i];
    }
    const G1 getSubshare(unsigned short i) const
    {
        return subshares[i];
    }
    string getHash(unsigned short i) const
    {
        return hashes[i];
    }

    const vector<NodeID> getIndices() const
    {
        return indices;
    }
    const vector<G1> getShares() const
    {
        return shares;
    }
    const vector<string> getHashes() const
    {
        return hashes;
    }
    const vector<G1> getSubshares() const
    {
        return subshares;
    }
